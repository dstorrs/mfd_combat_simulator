...
If a fighter has (e.g.) 6 dice then they generate at most 6 hits and can block at most 6 hits.
...
NOTE: When we stop, the full stats of all combatants will be written to the "./Heroes-final.csv" and "./Villains-final.csv" files.  In order to simulate, e.g., adding combatants or healing people mid-fight, run the combat for N rounds, modify the *-final.csv files, and run it again using those as your inputs.
...
```
    Name,     XP,     BonusXP, Wounds, BonusHP, BonusToHit, BonusToDefend, AOE, BuffWhichAllies, BuffAlliesOffense, BuffAlliesDefense, LinkedTo, BodyguardFor
    Dragon,  2000,   100,       0,      0,       0.1,        0,             3,  "Rider1,Rider2",          0.2,               0,                         , 
    Rider1,  1000,   10,        0,      1,       0.2,        0,             1,  "Rider2",                 0.1,               0.7,               Dragon   , Dragon
    Rider2,  500,    77,        1,      0,       0,          0,             1,  "Rider3",                 0.05,              0,                 Dragon   , 
